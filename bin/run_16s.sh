#!/usr/bin/env bash
# Purpose: one-command QIIME2 amplicon pipeline (16S workflow driver).
#
#   bash run_16s.sh --input /data/fastq --region 16S_V4 --classifier /db/silva138.qza
...
# quality profile, metadata auto-generated if absent, outputs into the project
# directory).
#
# A YAML config file is supported in addition to CLI flags:
#   --config FILE.yaml      read parameters from a YAML config (see
#                           config/16s_config.example.yaml). CLI flags
#                           override values from the file.
#
# Options (override config.sh):
#   --input DIR             directory containing *.fastq.gz / *.fq.gz (top level)
...
#   --trim-left-f N         DADA2 trim-left forward (manual override)
#   --trim-left-r N         DADA2 trim-left reverse (manual override)
#   --trunc-len-f N         DADA2 trunc-len forward (manual override)
#   --trunc-len-r N         DADA2 trunc-len reverse (manual override)
#   --expected-amplicon-len N  insert length WITHOUT primers (overlap check)
...
#   --init-only             only write the per-project config, then exit
#   --no-init               do not (re)write the per-project config
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
WORKFLOW_DIR="$(cd "${SCRIPT_DIR}/../workflows/16s" && pwd)"
...
# ---- pre-scan: --config FILE.yaml (values act as defaults; CLI wins) --------
CONFIG_FILE=""
_pending=""
for _a in "$@"; do
...
if [[ -n "${CONFIG_FILE}" ]]; then
  ...
  eval "$(python3 "${WORKFLOW_DIR}/../../bin/yaml2env.py" 16s "${CONFIG_FILE}")"
  echo "Loaded config file: ${CONFIG_FILE}"
fi
# remember values provided by the config file so they survive the CLI
# resolution below (CLI > config file > config.sh default)
TRIM_LEFT_F_CFG="${TRIM_LEFT_F:-}"
TRIM_LEFT_R_CFG="${TRIM_LEFT_R:-}"
TRUNC_LEN_F_CFG="${TRUNC_LEN_F:-}"
TRUNC_LEN_R_CFG="${TRUNC_LEN_R:-}"
MAX_EE_CFG="${MAX_EE:-}"

while [[ $# -gt 0 ]]; do
  case "$1" in
    --config)             shift 2 ;;
    --input)              FASTQ_DIR="${2:?missing value for --input}"; shift 2 ;;
    --output)             PROJECT_DIR="${2:?missing value for --output}"; shift 2 ;;
...
[[ "${PROJECT_DIR}" == "/path/to/qiime2_project" ]] && {
  echo "ERROR: set --output (or PROJECT_DIR in config)" >&2; exit 1; }
[[ -d "${FASTQ_DIR}" ]] || { echo "ERROR: FASTQ_DIR not found: ${FASTQ_DIR}" >&2; exit 1; }

# ---- resolve region -> primers ---------------------------------------------
if [[ -n "${REGION}" && -f "${WORKFLOW_DIR}/primers.tsv" ]]; then
...
# ---- write per-project config (into the PROJECT_DIR, never the toolkit dir) --
mkdir -p "${PROJECT_DIR}/work"
if [[ "${NO_INIT}" != "yes" ]]; then
  CFG="${PROJECT_DIR}/work/config.local.sh"
  {
    echo "# Auto-generated by run_16s.sh on $(date '+%F %T')"
    echo "# Per-project overrides; edit here or pass flags / --config to run_16s.sh."
    printf 'CONDA_SH=%q\n' "${CONDA_SH}"
...
# Per-project DADA2 parameter overrides.  Resolution: explicit CLI flag >
# config-file value (--config) > config.sh default.  Stale values in any old
# config.local.sh are intentionally ignored so projects never pollute each
# other.
TRIM_LEFT_F="${TRIM_LEFT_F_CLI:-${TRIM_LEFT_F_CFG:-${TRIM_LEFT_F_DEFAULT}}}"
TRIM_LEFT_R="${TRIM_LEFT_R_CLI:-${TRIM_LEFT_R_CFG:-${TRIM_LEFT_R_DEFAULT}}}"
TRUNC_LEN_F="${TRUNC_LEN_F_CLI:-${TRUNC_LEN_F_CFG:-${TRUNC_LEN_F_DEFAULT}}}"
TRUNC_LEN_R="${TRUNC_LEN_R_CLI:-${TRUNC_LEN_R_CFG:-${TRUNC_LEN_R_DEFAULT}}}"
MAX_EE="${MAX_EE_CLI:-${MAX_EE_CFG:-${MAX_EE_DEFAULT}}}"
{
  echo "# Auto-generated by run_16s.sh on $(date '+%F %T')"
  echo "# Per-project DADA2 parameter overrides. Edit here or pass flags to run_16s.sh."
...
if [[ "${DO_INIT}" == "yes" ]]; then
  ...
  echo "Next: bash ${SCRIPT_DIR}/run_16s.sh   (or edit ${PROJECT_DIR}/work/config.local.sh first)"
  exit 0
fi

mkdir -p "${PROJECT_DIR}/results" "${PROJECT_DIR}/work/tmp"
# Export the project dir so step scripts can find <project>/work/config.local.sh
# before sourcing it (config.sh keeps the exported value).
export PROJECT_DIR FASTQ_DIR
...
echo "=== run_16s.sh started $(date '+%F %T') ===" | tee -a "${LOG}"
...
run_step() {
  ...
  if ! bash "${WORKFLOW_DIR}/steps/${script}" 2>&1 | tee -a "${LOG}"; then
